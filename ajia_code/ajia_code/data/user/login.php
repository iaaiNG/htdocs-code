...
$sql = "SELECT uid FROM xz_user WHERE uname='$uname' AND upwd=md5('$upwd')";
...
if(!$result){       //SQL语句执行失败
  ...
  if(!$row){        //用户名或密码错误
    ...
    if($pageToJump==='CartPage' && @$_SESSION['toBuyLid']){
      //完成购物车添加
      ...
      if( mysqli_fetch_row($result) ){
        ...
        $sql = "INSERT INTO xz_shopping_cart VALUES(NULL, $_SESSION[loginUid], $_SESSION[toBuyLid], $_SESSION[toBuyCount],false)";
      }
      $result = mysqli_query($conn, $sql);
